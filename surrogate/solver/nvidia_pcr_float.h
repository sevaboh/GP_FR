...
__kernel void pcr_kernel1(__global float *a, __global float *b, __global float *c, __global float *d, __global float *a2, __global float *b2, __global float *c2, __global float *d2,\n\
...
__kernel void pcr_kernel2(__global float *a, __global float *b, __global float *c, __global float *d, __global float *x, \n\
									  int system_size, int delta)\n\
{\n\
    int thid = get_global_id(0);\n\
	if (thid < delta)\n\
...
void pcr_small_systems_clear()
{
	if (init)
	{
	    clReleaseKernel(pcrKernel[0]);
	    clReleaseKernel(pcrKernel_gl[0]);
	    clReleaseKernel(pcrKernel_gl[1]);
	    init=0;
	}
}
int pcr_small_systems_init_program(cl_context cxGPUContext,int id)
{
	cl_int errcode;
    ...
    // create program
    cpProgram = clCreateProgramWithSource(cxGPUContext, 1, (const char **)&s, &program_length, &errcode);
...
int pcr_small_systems_init_kernels(cl_context cxGPUContext,int id)
{
	cl_int errcode;
	size_t program_length=strlen(pcr_small_kernels);
	if (init)
